<b-card>
    ...
    if ($participant->qualis()->whereHas('requirements')->exists()) {
        $fields[__('t.models.quali.requirement_progress')] = function(\App\Models\Quali $quali) {
            if ($quali->requirements->count() === 0) return '';
            return (new App\Util\HtmlString)->s('<requirement-progress v-if="" :requirements="')->e(json_encode($quali->requirements))->s('"></quali-requirements>');
        };
    }
