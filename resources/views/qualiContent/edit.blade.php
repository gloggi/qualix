...
@section('content')
    ...
    <b-card body-class="container-fluid">
        ...
        <form-quali-content
            :action="['qualiContent.update', { course: {{ $course->id }}, participant: {{ $participant->id }}, quali: {{ $quali->id }}, noFormRestoring: 1 }]"
            course-id="{{ $course->id }}"
            ...
            :collaboration-key="{{ json_encode(env('COLLABORATION_ENABLED') ? $quali->collaborationKey : null) }}">
